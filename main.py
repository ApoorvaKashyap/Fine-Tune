...
def infoShow(trackName):
    ...
    try:
        ...
        duration_label.config(
            text="{}: {} min {} s".format(
                duration, int(int(line[4]) / 60), int(line[4]) % 60
            )
        )
...
lbl_Styles = Style()
lbl_Styles.configure(
    "W.TLabel", font=("Times", 14, "bold"), foreground="#ffffff", background="#50544f"
)
...
volume_scale = tk.Scale(
    root, from_=0, to=100, orient=tk.HORIZONTAL, command=vol_change, bg="#50544f"
)
